...
$(document).ready(function () {
    ...
    $("#search").on("click",function(){ 
        filter.page_name   = $('#page_name_search').val();
        if(filter.page_name == ''){
            $("#page_name_search_error").html('Please enter page Name').fadeIn().delay(3000).fadeOut();
            $('#page_name_search').focus();
            return false;
        }
        filter.start     = start;
        filter.limit     = limit;
    ...
    $("#page_access_form").validate({
        ...
        submitHandler: function (form) {
            queryString = $('#page_access_form').serialize();
            $("#submit_button").hide();
            $("#display_processing").css('display','inline-block');
            $.post(SITE_URL+'page-access-name/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'page-access-name');
            }, "json");
            return false;
        },
...
function statusUpdate(status,id,checkbox) {
    var url = SITE_URL + 'page-access-name/status/' + status+'/'+id;
    commonConfirmation(url, SITE_URL + 'page-access-name');
}
...
function filtering(filter) {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: SITE_URL + 'page-access-name/filter',
        data: filter,
