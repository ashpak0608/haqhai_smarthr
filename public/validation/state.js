...
$(document).ready(function () {
    ...
    $('#search_limits').on('change', function () {
     ...
     element.on('click', '#pagination ul li.filter', function(){
        ...
        if (newStart >= 0) {
            ...
            filter.limit = limit;
            filter.state_name   = $('#state_name').val();
            filter.country_id   = $('#country_id').val();
            filtering(filter);
        }
    });
    ...
    $("#search").on("click",function(){ 
        filter.state_name   = $('#state_name').val();
        filter.country_id   = $('#country_id').val();
        if(filter.state_name == '' && filter.country_id == ''){
            $("#country_id_error").html('Please select country name').fadeIn().delay(3000).fadeOut();
            $("#state_name_error").html('Please enter state name').fadeIn().delay(3000).fadeOut();
            $('#state_name').focus();
            return false;
        }
        filter.start     = start;
    ...
    $("#states_form").validate({
        ...
        submitHandler: function (form) {
            queryString = $('#states_form').serialize();
            $("#submit_button").hide();
            $("#display_processing").css('display','inline-block');
            $.post(SITE_URL+'state/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'state');
            }, "json");
            return false;
        },
...
function statusUpdate(status,id,checkbox) {
    var url = SITE_URL + 'state/status/' + status+'/'+id;
    let indexUrl = SITE_URL+'state';
            ...
            if (obj.status == 'success') {
                ...
                if (obj.total_count > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        let sr_no = i + 1 + start;
                        let add = SITE_URL + 'state/add/' + list[i]['id'];
                        let view = SITE_URL + 'state/view/' + list[i]['id'];
