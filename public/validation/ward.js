...
$(document).ready(function () {
    ...
    $("#search").on("click",function(){ 
        ...
        filter.ward_name   = $('#ward_name').val();
        if(filter.city_id == '' && filter.ward_name == ''){
            $("#city_id_error").html('Please enter city Name').fadeIn().delay(3000).fadeOut();
            $("#ward_name_error").html('Please select ward Name').fadeIn().delay(3000).fadeOut();
            $('#city_id').focus();
            return false;
        }
        filter.start     = start;
    ...
    $("#district_form").validate({
        ...
        submitHandler: function (form) {
            queryString = $('#ward_form').serialize();
            $("#submit_button").hide();
            $("#display_processing").css('display','inline-block');
            $.post(SITE_URL+'ward/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'ward');
            }, "json");
            return false;
        },
...
function statusUpdate(status,id,checkbox) {
    var url = SITE_URL + 'ward/status/' + status+'/'+id;
    let indexUrl = SITE_URL+'ward';
            ...
            if (obj.status == 'success') {
                ...
                if (obj.total_count > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        let sr_no = i + 1 + start;
                        let add = SITE_URL + 'ward/add/' + list[i]['id'];
                        let view = SITE_URL + 'ward/view/' + list[i]['id'];
                        html += `<tr id="follow_up_row${i}">
                                    <td>${sr_no}</td>
                                    <td>${list[i]['city_name']}</td>
                                    <td>${list[i]['ward_name']}</td>
                                    <td>${list[i]['ward_number']}</td>
                                    <td>
