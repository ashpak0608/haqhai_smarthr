...
$(document).ready(function () {
    ...
    $('#search_limits').on('change', function () {
        ...
        filter.start = start;
        filter.limit = limit;
        filter.role_name   = $('#role_name').val();
        filtering(filter);
    });
    ...
    $("#search").on("click",function(){ 
        filter.role_name   = $('#role_name').val();
        if(filter.role_name == ''){
            $("#role_name_error").html('Please enter Role Name').fadeIn().delay(3000).fadeOut();
            $('#role_name').focus();
            return false;
        }
        filter.start     = start;
    ...
    $("#role_form").validate({
        ...
        submitHandler: function (form) {
            ...
            $.post(SITE_URL+'role/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'role');
            }, "json");
            return false;
        },
...
function statusUpdate(status, id, checkbox) {
    var url = SITE_URL + 'role/status/' + status+'/'+id;
    let indexUrl = SITE_URL+'role';
            ...
            if (obj.status == 'success') {
                ...
                if (obj.total_count > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        let sr_no = i + 1 + start;
                        let add = SITE_URL + 'role/add/' + list[i]['id'];
                        let view = SITE_URL + 'role/view/' + list[i]['id'];
