...
$(document).ready(function () {
    ...
    $("#search").on("click",function(){ 
        filter.country_name   = $('#country_name').val();
        if(filter.country_name == ''){
            $("#country_name_error").html('Please enter Country Name').fadeIn().delay(3000).fadeOut();
            $('#country_name').focus();
            return false;
        }
        filter.start     = start;
    ...
    $("#country_form").validate({
        ...
        submitHandler: function (form) {
            queryString = $('#country_form').serialize();
            $("#submit_button").hide();
            $("#display_processing").css('display','inline-block');
            $.post(SITE_URL+'country/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'country');
            }, "json");
            return false;
        },
...
function statusUpdate(status, id, checkbox) {
    var url = SITE_URL + 'country/status/' + status+'/'+id;
    let indexUrl = SITE_URL+'country';
...
$("#export_data").on("click", function() {
    ...
    window.location.href = SITE_URL + 'country/export?' + params.toString();
})