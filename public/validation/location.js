...
$(document).ready(function () {
    ...
    $("#pin_location_form").validate({
        ...
        submitHandler: function (form) {
            queryString = $('#pin_location_form').serialize();
            $("#submit_button").hide();
            $("#display_processing").css('display','inline-block');
            $.post(SITE_URL+'location/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'location');
            }, "json");
            return false;
        },
...
function statusUpdate(status, id, checkbox) {
    var url = SITE_URL + 'location/status/' + status+'/'+id;
    let indexUrl = SITE_URL+'location';
            ...
            if (obj.status == 'success') {
                ...
                if (obj.total_count > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        let sr_no = i + 1 + start;
                        let add = SITE_URL + 'location/add/' + list[i]['id'];
