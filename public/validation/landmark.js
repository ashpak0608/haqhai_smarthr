...
$(document).ready(function () {
    ...
    $("#search").on("click",function(){ 
        filter.area_id   = $('#area_id').val();
        filter.landmark_name   = $('#landmark_name').val();
        if(filter.area_id == '' && filter.landmark_name == '') {
            $("#area_id_error").html('Please select area').fadeIn().delay(3000).fadeOut();
            $("#landmark_name_error").html('Please enter landmark name').fadeIn().delay(3000).fadeOut();
            return false;
        }
        filter.start     = start;
        filter.limit     = limit;
    ...
    $("#landmark_form").validate({
        ...
        submitHandler: function (form) {
            queryString = $('#landmark_form').serialize();
            $("#submit_button").hide();
            $("#display_processing").css('display','inline-block');
            $.post(SITE_URL+'landmark/save', queryString, function (data) {
               commonStatusMessage(data, SITE_URL+'landmark');
            }, "json");
            return false;
        },
...
function statusUpdate(status, id, checkbox) {
    var url = SITE_URL + 'landmark/status/' + status+'/'+id;
    let indexUrl = SITE_URL+'landmark';
            ...
            if (obj.status == 'success') {
                ...
                if (obj.total_count > 0) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        let sr_no = i + 1 + start;
                        let add = SITE_URL + 'landmark/add/' + list[i]['id'];
                        let view = SITE_URL + 'landmark/view/' + list[i]['id'];
                        html += `<tr id="follow_up_row${i}">
                                    <td>${sr_no}</td>
                                    <td>${list[i]['area_name']}</td>
                                    <td>${list[i]['landmark_name']}</td>
                                    <td>
